...
public class TournamentObserver extends Observer implements Runnable{
	...
	public TournamentObserver(String tournamentLogFolderPath, ArrayList<ScoreCalculator> scoreCalculators, int numGames, int numParticipants) throws IOException {
		...
		this.roundResults = new File(tournamentLogFolderPath, "roundResults.log");
		this.roundResults.createNewFile();
	...
	public void afterOldPhase() {
        ...
        for(Power power : game.getPowers()){
            if(game.getPhase().toString().equals("WIN"))
                FileIO.appendToFile(this.roundResults, power.getName() + ": " + power.getOwnedSCs().size());
        }
    }
...
//		FileIO.appendToFile(this.gameResultsFile, "game " + this.gameNumber + ": " + gameResult.toString());
